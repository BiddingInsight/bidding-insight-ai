...
import { SuspendedEntity } from '../types';
import { Spinner } from '../components/Spinner';

export const DebarmentPage: React.FC = () => {
  const [entities, setEntities] = useState<SuspendedEntity[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      const suspendedEntities = await db.getSuspendedEntities();
      setEntities(suspendedEntities);
      setIsLoading(false);
    };
    loadData();
  }, []);
    ...
    <div className="py-12">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-brand-blue-dark">Debarment & Suspension List</h1>
